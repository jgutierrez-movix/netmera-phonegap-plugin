...
- (void)initNetmera:(CDVInvokedUrlCommand *)command                    // set Netmera apiKey
{
...
- (void)overrideTags: (CDVInvokedUrlCommand *)command
{
    NMDeviceDetail *deviceDetail = [[NMDeviceDetail alloc ]init];
    NSArray *NMtags = [command.arguments objectAtIndex:0];
    [deviceDetail overrideTagsWithNewTags:NMtags];                       //deviceDetail - must take array value
...
{
    NSDictionary *NMdeviceDetailDic = [[NSDictionary alloc]init];
    ...
    else if (deviceDetail.customDictionary)
    {
        [NMdeviceDetailDic setValue:deviceDetail.customDictionary forKey:@"Custom dictionary"];
    }
    else if (deviceDetail.getTags)
    {
...
- (void)getAvailableTags:(CDVInvokedUrlCommand *)command  // returns an array
{
...
- (void)sendEventWithKey: (CDVInvokedUrlCommand *)command                  // "value" must take dictionary value and "key" takes string
{
